...
void draw() {
  lights();
  background(0);
  float far = map(mouseX, 0, width, 20, 400);
  if (showPerspective == true) {
    perspective(PI/3.0, float(width)/float(height), 100, far);
  } else {
    ortho(-width/4, width/4, -height/2.0, height/2.0, 10, far);
  }
...
//Se cambio el rango de distancia mínimo es más cercano.
//Cambiaste el límite izquierdo de -width/2.0 a -width/4.
//y cambie 
